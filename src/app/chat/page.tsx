...
const KEYWORD_SUGGESTIONS = {
  ending: ['HE', 'BE', '开放式', '反转式', '虐恋', '破镜重圆', '单向奔赴'],
  relation: ['强强对抗', '权力博弈', '推拉试探', '信任危机', '征服与臣服', '亦敌亦友', '宿敌变情人', '禁忌拉扯', '师徒逆转', '沉默的占有欲'],
  scene: ['审讯室对峙', '酒吧最后一盏灯', '雨夜追车', '废弃工厂', '病床前对望', '葬礼重逢', '雪夜旅馆', '深夜电话', '宴会上的对视'],
  stage: ['第一幕·奠基', '第二幕·攀升', '第三幕·高潮', '终幕·余波'],
};
...
export default function ChatPage() {
  ...
  // Keyword library states (suggestions + free input)
          ...
          <div className="p-3 space-y-3 max-h-[55vh] overflow-y-auto">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                {currentMainLine && (
                  <button
                    onClick={() => { setCurrentMainLine(''); setPlotStage(''); setProgressDesc(''); }}
                    className="text-[10px] text-gray-400 hover:text-gray-600"
                  >清除</button>
                )}
              </div>
              {currentMainLine ? (
                <div className="p-2.5 rounded-lg border bg-amber-50/50 border-amber-200">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {plotStage && <span className="text-[10px] bg-amber-100 text-amber-600 px-1.5 py-0.5 rounded">{plotStage}</span>}
                  </div>
                  {progressDesc && <p className="text-[11px] text-gray-500">{progressDesc}</p>}
                  <button
                    onClick={() => { setCurrentMainLine(''); setPlotStage(''); setProgressDesc(''); }}
                    className="text-[10px] text-amber-500 hover:text-amber-700 mt-1"
                  >解锁/更换主线</button>
                </div>
              ) : (
                <div className="space-y-1.5">
                  <input
                    ...
                    placeholder="输入主线名称（选择预测后自动填入，或手动输入）"
                    className="w-full text-[11px] px-2.5 py-1.5 rounded-lg border border-amber-100 bg-amber-50/30 focus:border-amber-300 focus:outline-none placeholder:text-amber-300"
                  />
                  <input
                    type="text"
                    value={plotStage}
                    onChange={e => setPlotStage(e.target.value)}
                    placeholder="当前阶段（可选，如：第二幕·攀升）"
                    className="w-full text-[11px] px-2.5 py-1.5 rounded-lg border border-amber-100 bg-amber-50/30 focus:border-amber-300 focus:outline-none placeholder:text-amber-300"
                  />
                  <textarea
                    value={progressDesc}
                    ...
                    placeholder="进展描述（可选）"
                    className="w-full text-[11px] px-2.5 py-1.5 rounded-lg border border-amber-100 bg-amber-50/30 focus:border-amber-300 focus:outline-none placeholder:text-amber-300 resize-none"
                    rows={2}
                  />
                </div>
              )}
            ...
            <div>
              <span className="text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-1.5 block">关键词库</span>
              <div className="space-y-2">
                {/* Ending keywords */}
                <div>
                  <span className="text-[10px] text-amber-500 font-medium">结局走向</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {KEYWORD_SUGGESTIONS.ending.map(kw => (
                      <button
                        key={kw}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {KEYWORD_SUGGESTIONS.relation.map(kw => (
                      <button
                        key={kw}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {KEYWORD_SUGGESTIONS.scene.map(kw => (
                      <button
                        key={kw}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {KEYWORD_SUGGESTIONS.stage.map(kw => (
                      <button
                        key={kw}
